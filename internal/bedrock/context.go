...
func Snapshot(root, task string, maxFiles, maxBytes int) ([]FileContext, error) {
	if maxFiles <= 0 { maxFiles = 80 }
	if maxBytes <= 0 { maxBytes = 200 * 1024 }
	...
	err = filepath.WalkDir(root, func(path string, d fs.DirEntry, walkErr error) error {
		...
		if d.IsDir() {
			if _, skip := ignoredDirs[d.Name()]; skip { return filepath.SkipDir }
			return nil
		}
		if len(candidates) >= maxCandidates || scanned >= maxScannedBytes { return nil }
		if sensitiveContextPath(d.Name()) { return nil }
		info, err := d.Info()
		if err != nil || !info.Mode().IsRegular() || info.Size() > maxPerFile { return nil }
		data, err := os.ReadFile(path)
		if err != nil { return nil }
		if bytes.IndexByte(data, 0) >= 0 { return nil }
		if scanned+len(data) > maxScannedBytes { return nil }
		scanned += len(data)
		rel, err := filepath.Rel(root, path)
		if err != nil { return nil }
		rel = filepath.ToSlash(rel)
		text := string(data)
		candidates = append(candidates, candidateFile{path: rel, content: text, score: contextScore(rel, text, terms)})
		return nil
	})
	if err != nil { return nil, fmt.Errorf("scan repository: %w", err) }
...
func sensitiveContextPath(name string) bool {
	...
	if name == ".env" || strings.HasPrefix(name, ".env.") { return true }
	switch name {
	case ".netrc", ".npmrc", ".pypirc", "credentials.json", "service-account.json", "id_rsa", "id_dsa", "id_ecdsa", "id_ed25519":
		return true
	}
...
func contextScore(path, content string, terms []string) int {
	...
	switch base {
	case "readme.md", "go.mod", "package.json", "pom.xml", "pyproject.toml", "cargo.toml": score += 5
	}
	for _, term := range terms {
		if strings.Contains(p, term) { score += 8 }
		if strings.Contains(c, term) { score += 2 }
	}
	return score
}