...
func (passingVerifier) Verify(_ context.Context, _ string) ([]VerificationResult, error) {
	return []VerificationResult{{Command: "check", ExitCode: 0, Output: "ok"}}, nil
}
...
func TestEngineRepairsAfterFailedVerification(t *testing.T) {
	...
	if result.Evidence.Status != "CHECKS_PASSED" || provider.calls != 2 {
		t.Fatalf("status=%s provider calls=%d", result.Evidence.Status, provider.calls)
	}
	if len(result.Evidence.BaselineVerification) != 1 || result.Evidence.BaselineVerification[0].ExitCode == 0 {
		t.Fatalf("baseline verification=%+v", result.Evidence.BaselineVerification)
	}
	if len(result.Evidence.Verification) != 1 || result.Evidence.Verification[0].ExitCode != 0 {
		t.Fatalf("post verification=%+v", result.Evidence.Verification)
	}
	if len(result.Evidence.ProviderSummaries) != 2 || result.Evidence.ProviderSummaries[1] != "repair" {
		t.Fatalf("provider summaries=%v", result.Evidence.ProviderSummaries)
	}
...
func TestEngineAlreadyGreenNoOpDoesNotClaimTaskCompletion(t *testing.T) {
	...
	if result.Evidence.Status != "CHECKS_PASSED" {
	...
	if len(result.Evidence.BaselineVerification) != 1 || len(result.Evidence.Verification) != 1 {
		t.Fatalf("baseline=%v post=%v", result.Evidence.BaselineVerification, result.Evidence.Verification)
	}
	if len(result.Evidence.ProviderSummaries) != 1 || result.Evidence.ProviderSummaries[0] != "no changes needed" {
		t.Fatalf("provider summaries=%v", result.Evidence.ProviderSummaries)
	}
}
...
func TestEngineRollsBackWhenVerificationNeverPasses(t *testing.T) {
	...
	if err == nil || !result.Evidence.RolledBack {
		t.Fatalf("expected failed rollback, err=%v evidence=%+v", err, result.Evidence)
	}
...
func TestEngineRejectsNonzeroVerificationResultWithoutError(t *testing.T) {
	...
	if err == nil || result.Evidence.Status == "CHECKS_PASSED" || !result.Evidence.RolledBack {
...
func TestEngineReportsRollbackConflictWithoutOverwritingConcurrentEdit(t *testing.T) {
	...
	if result.Evidence.RolledBack {
		t.Fatal("rollback conflict recorded as successful")
	}
	if !strings.Contains(result.Evidence.LastFailure, "rollback failed") || !strings.Contains(result.Evidence.LastFailure, "changed after BedRock wrote it") {
		t.Fatalf("rollback conflict missing: %q", result.Evidence.LastFailure)
	}
	data, readErr := os.ReadFile(path)
...
func TestEngineRejectsTargetChangedDuringProviderExecution(t *testing.T) {
	...
	if !result.Evidence.RolledBack {
		t.Fatal("expected no-op rollback")
	}
	data, readErr := os.ReadFile(path)
	if readErr != nil || string(data) != "external edit" {
		t.Fatalf("external edit overwritten: %q err=%v", data, readErr)
	}
}
...
func TestEnginePropagatesCancellationToProvider(t *testing.T) {
	...
	go func() { _, err := engine.Run(ctx, root, "wait for cancellation"); done <- err }()
	select {
	case <-started:
		cancel()
